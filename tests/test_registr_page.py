...
def test_page_left_registration(selenium):
    """RT-11 Левая часть формы «Регистрация» содержит логотип и продуктовый слоган кабинета."""
    try:
...
def test_elements_of_registr(selenium):
    """RT-12 Блок регистрации формы «Регистрация» содержит основные элементы:
...
def test_names_elements_of_registr(selenium):
    """RT-13 Названия элементов блока регистрации формы «Регистрация» соответствуют Требованию."""
    try:
        page_reg = RegistrPage(selenium)
...
def test_registr_by_valid_data(selenium):
    """RT-14 Регистрация пользователя с валидными данными"""
    page_reg = RegistrPage(selenium)
...
def test_registr_by_valid_data_(selenium):
    """RT-15 Регистрация пользователя с валидными данными:
...
def test_registr_by_invalid_data(selenium):
    """RT-16 Регистрация пользователя по email, который уже был использован ранее для регистрации"""
    page_reg = RegistrPage(selenium)
    page_reg.first_name.send_keys(Settings.f_name)
    page_reg.first_name.clear()
    page_reg.last_name.send_keys(Settings.l_name)
...
def test_registr_and_redir_auth(selenium):
    """RT-17 Регистрация пользователя по email, который уже был использован ранее для регистрации
...
def test_first_name_by_valid_data(selenium, valid_first_name):
    """RT-18 Проверка поля ввода "Имя" формы «Регистрация» валидными данными:
...
def test_first_name_by_invalid_data(selenium, invalid_first_name):
    """RT-19 Проверка поля ввода "Имя" формы «Регистрация» невалидными данными:
    пустое значение;
...
def test_last_name_by_valid_data(selenium, valid_password):
    """RT-20 Проверка поля ввода "Пароль" формы «Регистрация» валидными данными:
...
def test_passw_registration_confirm_valid_data(selenium):
    """RT-21 Проверка поля ввода "Пароль" и "Подтвердить пароль" формы «Регистрация» валидными данными
    (пароли совпадают)."""
    page_reg = RegistrPage(selenium)
...
def test_passw_registration_confirm_invalid_data(selenium):
    """RT-22 Проверка поля ввода "Пароль" и "Подтвердить пароль" формы «Регистрация» невалидными данными
    (пароли не совпадают)."""
    page_reg = RegistrPage(selenium)
    page_reg.passw_registration.send_keys(Settings.passw1)
    page_reg.passw_registration.clear()
