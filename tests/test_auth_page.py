...
def test_page_right(selenium):
    """RT-01 В правой части формы «Авторизация» находится продуктовый слоган ЛК "Ростелеком ID"."""
    try:
...
def test_elements_of_auth(selenium):
    """RT-02 Блок аутентификации формы «Авторизация» содержит основные элементы (Меню выбора типа аутентификации,
...
def test_menu_of_type_auth(selenium):
    """RT-03 Меню выбора типа аутентификации содержит табы: 'Номер', 'Почта', 'Логин', 'Лицевой счёт'."""
    try:
        page = AuthPage(selenium)
...
def test_menu_of_type_active_auth(selenium):
    """RT-04 В Меню выбора типа аутентификации по умолчанию выбрана форма аутентификации по телефону."""
    page = AuthPage(selenium)
...
def test_placeholder_name_of_user(selenium):
    """RT-05 В форме ввода ('Номер', 'Почта', 'Логин', 'Лицевой счёт') плейсхолдер меняется в соответствии с
...
def test_forgot_password_link(selenium):
    """RT-06 Проверка перехода по ссылке 'Забыл пороль'."""
    page = AuthPage(selenium)
...
def test_register_link(selenium):
    """RT-07 Проверка перехода по ссылке 'Зарегистрироваться'."""
    page = AuthPage(selenium)
...
def test_auth_by_valid_email_pass(selenium):
    """RT-08 Аутентификация пользователя с валидным email и паролем."""
    page = AuthPage(selenium)
...
def test_auth_by_invalid_email(selenium, incor_email, incor_passw):
    """RT-09, RT-10 Аутентификация пользователя с невалидным email и паролем:
